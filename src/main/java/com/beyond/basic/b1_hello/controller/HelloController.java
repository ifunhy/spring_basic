...
//        String result = objectMapper.writeValueAsString(h1);
    ...
    @GetMapping("/param")
    @ResponseBody
    // RequestParam : 파라미터로 들어오면, ?name : 으로 들어오면 inputName에 값을 넣어주겠다
    public Hello param(@RequestParam(value = "name")String inputName) {
        // 실습 : {name:사용자가넣은이름, email:아무거나}
    ...
    @GetMapping("/path/{inputId}")  // inputId 값이 들어오면 아래의 함수에 인자값으로 넣어주겠다
    @ResponseBody
    ...
    // case5. parameter 2개 이상인 형식
    // /hello/param2?name=hong&email=hong@naver.com 라고 get요청이 들어온다면
     @GetMapping("/param2")
    @ResponseBody
     // value = "" : value가 키값을 의미
...
//    public String param3(Hello hello) {
    // ModelAttribute를 써도 되고 안 써도 되는데, 이 키워드를 써서 명시적으로 param형식의 데이터를 받겠다는 것을 표현
    ...
    // 서버에서 화면(+데이터)을 렌더링해주는 ssr방식(csr의 서버는 데이터만)
    ...
    // post요청의 case들 : url인코딩방식 또는 multipart-formdata, json
    ...
    public String formFileViewPost(@ModelAttribute Hello hello,
                                   @RequestParam(value = "photo")MultipartFile photo) {    // RequestParam이 총 3개가 됨
        System.out.println(hello);
        System.out.println(photo.getOriginalFilename());    // 터미널에서 이미지 확인이 어려우니 파일명으로 확인
        return ("OK");
    }
    ...
    // case6. json + file 같이 처리할 때 : text 구조가 복잡하여 피치못하게 json을 써야하는 경우
