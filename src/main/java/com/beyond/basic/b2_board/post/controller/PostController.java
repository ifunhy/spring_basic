...
//    }
    ...
    // 페이징처리를 위한 데이터 요청 형식 : localhost:8080/post/list?page=0&size=20&sort=title,asc -> 파라미터를 받아주는 형식으로 들어감(url인코딩)
    // 요청을 ?& 라고 호출하고 있기 때문에 파라미터를 받아주는 형식인 것(url인코딩 또는 multipart-formdata)
    // Pageable 에서 받는 파라미터 : size, page
    // PostSearchDto 에서 받는 파라미터 : title, category
